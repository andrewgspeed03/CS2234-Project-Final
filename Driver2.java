...
public class Driver2 {
    ...
    public static void plotTrip(int seconds, ArrayList<TripPoint> trip, JMapViewer map) throws IOException {
        ...
        for (int i = 0; i < trip.size(); ++i) {
            ...
            BufferedImage arrow = rotateArrow(c1, c2);
    ...
    private static BufferedImage rotateArrow(Coordinate c1, Coordinate c2) throws IOException {
        double angle = 0.0;
        if (c2 != null) {
            ...
            angle = Math.atan2((lon1 - lon2) , (lat1 - lat2));

        }
        
        BufferedImage arrow = ImageIO.read(new File("arrow.png"));
        AffineTransform tx = new AffineTransform();
        tx.translate(arrow.getWidth() / 2, arrow.getHeight() / 2);
        tx.rotate(angle);
        tx.translate(-arrow.getWidth() / 2, -arrow.getHeight() / 2);
